...
export default function RecentLearningCard({ items }: RecentLearningCardProps) {
  ...
  return (
    <section className="rounded-[26px] border border-black/[0.075] bg-white p-5 shadow-[0_10px_36px_rgba(16,16,15,0.045)]">
      ...
      <div className="mt-5 space-y-2.5">
        {items.map((item) => (
          <Link
            key={item.articleId}
            href="/discover"
            className="flex items-center gap-3 rounded-[18px] bg-[#ebe7de] p-4 transition-transform active:scale-[0.99]"
          >
