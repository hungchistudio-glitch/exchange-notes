  ...
  return (
    ...
    >
      <div className="flex items-center justify-between gap-3">
        <span className="inline-flex items-center gap-1.5 text-[10px] font-semibold uppercase tracking-[0.14em] text-ink-faint">
          <Languages size={13} />
      ...
      <p className="mt-3 line-clamp-3 whitespace-pre-wrap break-words text-[15px] font-semibold leading-6 text-ink-strong">
        {note.originalText}
      </p>
      ...
      {note.tags.length ? (
        <div className="mt-3 flex flex-wrap gap-1.5">
          {note.tags.slice(0, 3).map((tag) => (
            <span key={tag} className="rounded-full bg-surface px-2.5 py-1 text-[10px] font-medium text-ink-soft">
              #{tag}
            </span>
          ))}
        </div>
