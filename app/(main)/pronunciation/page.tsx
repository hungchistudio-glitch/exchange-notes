    ...
    () =>
      learningChinese
        ? {
            ...
            description: "從聲母、韻母與聲調開始，建立正確的繁體中文發音基礎。",
    ...
    >
      <div className="mx-auto max-w-xl">
        <header className="grid grid-cols-[44px_1fr_44px] items-center">
          ...
          <p className="text-center text-[15px] font-semibold">Pronunciation</p>
    ...
    <div className="space-y-7">
      ...
      <PracticeCard
        ...
        lines={[
          ...
          [
            "2. Listen again and copy the mouth movement.",
            "再聽一次，模仿嘴型與舌頭位置。",
          ],
          [
            "3. Say the example slowly, then naturally.",
            "先慢速念範例，再用自然速度朗讀。",
          ],
          [
            "4. Compare similar sounds.",
            "比較容易混淆的相似音，例如 /iː/ 和 /ɪ/。",
          ],
        ]}
      />
    </div>
...
function EnglishSoundCard({ sound }: { sound: EnglishSound }) {
  return (
    <article className="flex min-h-[220px] flex-col justify-between rounded-[24px] border border-black/[0.06] bg-white p-5">
      ...
      <div className="mt-6">
        <div className="flex items-end justify-between gap-2">
          <div>
            <p className="text-[19px] font-semibold">{sound.example}</p>
        ...
        <div className="mt-4 border-t border-black/[0.06] pt-3">
          ...
          <p className="mt-1 text-[12px] text-black/30">{sound.cueChinese}</p>
        </div>
      </div>
    </article>
  );
    ...
    <div className="space-y-7">
      ...
      <PracticeCard
        ...
        lines={[
          [
            "1. 先認識聲母，再學習韻母。",
            "Learn the initials first, followed by the finals.",
          ],
          [
            "2. 將聲母與韻母慢慢拼合。",
            "Slowly combine the initial and final.",
          ],
          [
            "3. 最後加入聲調，完整念出整個音節。",
            "Add the tone and pronounce the complete syllable.",
          ],
          [
            "4. 每次練習都搭配真正的中文字。",
            "Always practise with a real Chinese character.",
          ],
        ]}
      />
    </div>
...
function ZhuyinSoundCard({ sound }: { sound: ZhuyinSound }) {
  return (
    <article className="flex min-h-[220px] flex-col justify-between rounded-[24px] border border-black/[0.06] bg-white p-5">
      <div className="flex items-start justify-between gap-3">
        <span
      ...
      <div className="mt-6">
        <div className="flex items-end justify-between gap-2">
          <div>
            <p className="text-[21px] font-semibold">{sound.example}</p>
...
function ToneCard({ tone }: { tone: ToneItem }) {
  return (
    <article className="flex items-center gap-4 rounded-[23px] border border-black/[0.06] bg-white px-5 py-4">
      ...
      <div className="min-w-0 flex-1">
        <p className="text-[16px] font-semibold">{tone.chineseLabel}</p>

        <p className="mt-0.5 text-[12px] text-black/35">{tone.englishLabel}</p>
...
function PracticeCard({
  ...
  return (
    <section className="rounded-[28px] bg-black p-6 text-white">
      ...
      <p className="mt-1 text-[13px] text-white/42">{translatedTitle}</p>

      <div className="mt-6 space-y-5">
        {lines.map(([primary, secondary]) => (
          <div key={primary}>
            <p className="text-[14px] leading-6 text-white/82">{primary}</p>
