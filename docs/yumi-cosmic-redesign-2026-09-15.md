...
參考來源為使用者提供的兩張 HUD 截圖：深紅於全黑、括號框、分段刻度、細長大字、極小的全大寫等寬標籤。取其形，不取其色與其字。

### 首頁：三格讀值改為一條儀表帶

原本是三張分開的圓角卡（單字數、待複習、學習語言）。改為單一面板，兩端各有一個括號記號，格與格之間以細線分隔而非間距——參考圖的叢集感來自這裡。

語言格移到中間，成為錶盤：圈內是該語言的 badge（`En`／`中`／`Es`／`Fr`／`It`，設定選單與 landing 已在用的同一組短碼），完整譯名放在圈外下方。名稱不放圈內是因為同一個語言在不同介面是「Traditional Chinese」或「Chino tradicional」，都塞不進圓圈。badge 標為 `aria-hidden`，否則輔助科技會把同一件事讀兩次。

兩側數字下方是刻度規，**沒有指針**。單字數沒有「滿格」可以當分母，對著捏造的最大值畫指針是儀表唯一不能做的事。每日目標那張卡可以有指針，因為目標真的有標的。

360px 以下三格加一個 3.4rem 錶盤排不下，錶盤獨佔上排、兩個數字並列下排。錶盤在標記中間，自動排版會把它推到第一個數字下方，因此三格都明確指定 grid 位置。

### 設定：粉彩保留，加上儀表細節

五張卡的薄荷／粉／藍／金底色不動——那是這次改造最接近原始參考影片的部分。加的是：對角兩個角落的括號記號，以卡片自身的深色墨繪製而非疊上第二種顏色；兩張計數卡（已精熟、已複習）補上與比率卡相同的刻度規，同樣不帶指針。

四個角落只取對角兩個：一對已足以說明「這是一個量測區域」，四個角在圓角卡上會變成一條猶豫不決的邊框。

### 標籤字體

刻意沒有回到參考圖的等寬全大寫小字。那正是第一輪必須從標籤移除的東西——繁體中文沒有大寫、等寬字沒有 CJK 字形。科技感由框、刻度與數字承擔，所有文字留在可讀的字體裡。

### 驗證

`tsc` 乾淨、`next build` 通過 57 頁、Vitest 133 檔 1,051 測試全過、ESLint 0 error。

瀏覽器檢查：320／360／430px；西班牙文＋大字級＋五位數數值＋最長譯名（Chino tradicional）無橫向溢位、無裁切；乾淨載入三次無 hydration 警告。新樣式沒有引入重複選擇器，也沒有留下無人引用的 class（以與第二輪相同的兩項檢查確認）。

## Progress HUD 改為可讀的量測儀表（第四輪）

第三輪給了五張卡儀表的外框，但刻度仍是裝飾——一排等距的細線，88% 和 0% 只差在上方那兩個字元。這一輪讓刻度真的能讀。

### 刻度分主副，數值以下點亮

每二十分之一一道短刻度、每四分之一一道全高刻度，兩道 repeating gradient 畫在同一個元素上，取卡片自身的墨色。點亮層是同一份刻度疊在上方，以 `clip-path` 裁到讀數為止——不是把它縮寬，因為縮寬會讓漸層百分比改以自身寬度解析，兩層刻度就會差半個像素對不齊。

比率卡（正確率、記憶保留）有真正的 0–100 刻度，因此有指針。計數卡（已精熟、已複習）沒有——「已精熟 4」沒有滿格可以當分母，對著無人設定的最大值畫指針，是儀表唯一不能做的事。新增測試以行內 `--progress` 屬性釘住這條不變條件，而不是以 class 名稱。

### 每日目標成為主角

原本是左側大數字配右側淡環，環在粉彩底上幾乎看不見。改為：六十道刻度、每五道一個主刻度，較粗的弧線帶柔光，讀數移進圈內。卡片的版面從左右兩欄改為置中單欄，錶盤是整個面板圍繞的那個儀器。

讀數不可用時只顯示破折號，不帶 `/10`——對著空值掛一個標的沒有意義。

### 動態

弧線與指針在讀數抵達的那一幀從零掃到定位，只做一次。弧線靠 transition（載入中的值本來就是零），指針與點亮層靠 `both` 填充的 keyframes，因此不需要任何 mount 狀態或額外渲染。`prefers-reduced-motion` 下動態關閉，數值照常顯示。

### 深度

卡片加上內凹高光與外投影，讀起來像加工面板而非色塊；角落記號放大並提高對比。刻度內縮至 `0.5rem`，避開角落記號——框框住刻度，不穿過它。

### 驗證

`tsc` 乾淨、`next build` 通過 57 頁、Vitest 133 檔 1,052 測試全過、ESLint 0 error。

瀏覽器檢查：430px 以使用者實際數值（1/10、88%、0%、4、114）確認指針落在 88% 位置、點亮區段止於該處、計數卡無指針；200%字級下錶盤受 `52vw` 上限約束、四張卡皆無裁切、無橫向溢位。ProgressHud 樣式表無重複選擇器、無無人引用的 class。
